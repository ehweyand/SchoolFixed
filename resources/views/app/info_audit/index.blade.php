...
<body>
    ...
    <div>
        <h1>Visualização de Auditoria do sistema</h1>
        {{-- Alterar a data das auditorias --}}


        <form action="{{ route('app.audit') }}">
            <label for="date"><b>Registros a partir de:</b> </label>
        ...
        <table>
            <tr>
                ...
                <th>IP</th>
                <th>Navegador</th>
                <th>Registro da auditoria</th>
            </tr>
            @foreach ($audits as $audit)
                <tr>
                    <td>{{ $audit->id }}</td>
                    <td>{{ $audit->event == 'created' ? 'Criação' : ($audit->event == 'updated' ? 'Atualização' : 'Exclusão') }}
                    </td>
                    <td>{{ $audit->auditable_id }}</td>
                    <td>{{ $audit->url }}</td>
                    <td class="green-text"><b>{{ $audit->ip_address }}</b></td>
